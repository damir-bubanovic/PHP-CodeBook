...
> The web-ls.php program provides a view of the files inside your web server’s document root, 
formatted like the output of the Unix command ls. 
> Filenames are linked so that you can download each file, and directory names are linked so
that you can browse in each directory
...
/*web-ls.php*/
/*Bit masks for determining file permissions and type. The names and values listed below are 
POSIX-compliant; individual systems may have their own extensions*/
...
/* mode_string() is a helper function that takes an octal mode and returns a 10-character string 
representing the file type and permissions that correspond to the octal mode. This is a PHP version 
of the mode_string() function in the GNU fileutils package.*/
$mode_type_map = array(
	S_IFBLK => 'b', 
	S_IFCHR => 'c',
	S_IFDIR => 'd', 
...
if (!(($real_dir == $real_docroot) || ((strlen($real_dir) > strlen($real_docroot)) &&
(strncasecmp($real_dir, $real_docroot . DIRECTORY_SEPARATOR, strlen($real_docroot . DIRECTORY_SEPARATOR)) == 0)))) {
...
foreach(new DirectoryIterator($real_dir) as $file) {
	...
	if(function_exists('posix_getpwuid')) {
		$user_ingo = posix_getpwuid($file->getOwner());
	} else {
	...
	if(($mode_type == 'c') || ($mode_type == 'b')) {
		...
		$statInfo = lstat($file->getPahtname());
